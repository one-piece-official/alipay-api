...
type RequestBody struct {
	// 公共参数
	AppID     string `json:"app_id" url:"app_id"`       // 应用 id
	Charset   string `json:"charset" url:"charset"`     // 请求使用的编码
	Format    string `json:"format" url:"format"`       // 格式，选填
	Method    string `json:"method" url:"method"`       // 接口名称
	SignType  string `json:"sign_type" url:"sign_type"` // 生成签名的算法
	Sign      string `json:"sign,omitempty" url:"sign"` // 签名
	Timestamp string `json:"timestamp" url:"timestamp"` // 时间
	Version   string `json:"version" url:"version"`     // 版本

	// 请求参数集合
	BizContent string `json:"biz_content,omitempty" url:"biz_content,omitempty"`
}
